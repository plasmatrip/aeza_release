        ...
        GestureDetector(
          ...
          onTap: () async {
            var status = await statusMenu(
              context: context,
              globalKey: globalKey,
              title: 'Текущий статус релиза',
              values: ReleaseStatus.values,
            );
            if (status != null && context.mounted) {
              context.read<ReleasesRepo>().status = status;
            }
          },
          child: Container(
            key: globalKey,
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    context.watch<ReleasesRepo>().status != null
                        ? Text(ReleaseStatus.values[context.read<ReleasesRepo>().status!].name, style: context.s14w400.copyWith(color: black))
                        : Text('Текущий статус релиза', style: context.s14w400.copyWith(color: grey_3)),
                    SvgPicture.asset(
